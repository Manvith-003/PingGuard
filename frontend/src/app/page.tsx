      ...
      <header className="sticky top-0 z-30 border-b border-white/5 bg-[#030712]/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center bg-white/5 rounded-full px-3 py-1.5 border border-white/10">
              <Clock className="w-4 h-4 text-blue-400 mr-2" />
              <select
                onChange={(e) => setIntervalHandler(Number(e.target.value))}
                className="bg-transparent text-sm font-medium border-none focus:outline-none appearance-none cursor-pointer pr-4"
                defaultValue=""
              >
                <option value="" className="bg-[#111827]">Set Interval</option>
                <option value={1} className="bg-[#111827]">1 min</option>
                <option value={5} className="bg-[#111827]">5 min</option>
                <option value={10} className="bg-[#111827]">10 min</option>
                <option value={15} className="bg-[#111827]">15 min</option>
                <option value={30} className="bg-[#111827]">30 min</option>
                <option value={60} className="bg-[#111827]">60 min</option>
              </select>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="glass-card p-6 rounded-2xl">
            <p className="text-slate-400 text-sm font-medium mb-1">Total Websites</p>
            <h2 className="text-3xl font-bold">{websites.length}</h2>
          </div>
          <div className="glass-card p-6 rounded-2xl border-l-4 border-l-green-500">
            <p className="text-slate-400 text-sm font-medium mb-1">Active / Up</p>
            <h2 className="text-3xl font-bold text-green-500">
              {websites.filter(s => s.status === 'UP').length}
            </h2>
          </div>
          <div className="glass-card p-6 rounded-2xl border-l-4 border-l-red-500">
            <p className="text-slate-400 text-sm font-medium mb-1">Outages / Down</p>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">Monitor List</h2>
          </div>
          
          <button
            onClick={handlePing}
            disabled={loading}
            className="flex items-center gap-2 text-sm font-medium bg-white/5 hover:bg-white/10 border border-white/10 px-3 py-1.5 rounded-lg transition-colors"
          >
            <RefreshCcw className={cn("w-4 h-4 text-blue-400", loading && "animate-spin")} />
            Sync Now
          </button>
        </div>
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-lg group-hover:text-blue-400 transition-colors">{site.name}</h3>
                      <p className="text-xs text-slate-500 truncate max-w-[150px]">{site.url}</p>
                    </div>
                  </div>
                  <button 
                ...
                <div className="grid grid-cols-2 gap-4 mt-6">
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Response Time</span>
                    <div className="flex items-center gap-1.5">
                      <Zap className="w-3.5 h-3.5 text-amber-400" />
                      <span className="text-sm font-semibold">{site.responseTime ? `${site.responseTime}ms` : "---"}</span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Last Check</span>
